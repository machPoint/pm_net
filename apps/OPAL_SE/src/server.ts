...
import { safelyCloseDatabase } from './config/database-shutdown';
...
import { MCP_VERSION, PROTOCOL_VERSION, SERVER_INFO, HEARTBEAT_INTERVAL, ERROR_CODES } from './config/constants';

// Server Port from environment or constants
const MCP_PORT = process.env.MCP_PORT || constants.MCP_PORT;

// Load handlers and utilities
import logger from './logger';
import { loadApiIntegrationsFromEnv } from './config/apiConfig';
import { executeApiTool } from './services/apiService'; // Corrected path
import { summarizeContent } from './services/summarizationService'; // Add summarization service
...
import * as elicitationService from './services/elicitationService';
...
import {
  ...
  getPromptArguments
} from './utils/completion'; // Completion utilities
...
const rpcServer = new JSONRPCServer<{ session: Session }>();

(global as any).sessions = new Map<string, Session>(); // WebSocket session state
const sessions = (global as any).sessions as Map<string, Session>; // Local reference
...
import { initializeOPALCore } from './examples/opal-core-integration-tmp';
...
initializeOPALCore(configs, wss).catch((error: any) => {
  logger.error('Failed to initialize OPAL Core:', error);
  // Continue without OPAL Core if it fails
});

// Initialize OPAL_SE Tools and Rule Engine
import { registerSETools } from './services/se/seToolsRegistration';
import { initializeRuleEngine } from './services/se/ruleEngineService';
...
Promise.resolve().then(async () => {
  try {
    ...
    // Register SE MCP tools
    await registerSETools(configs, wss);
    logger.info('SE tools registered');
  } catch (error: any) {
    logger.error('Failed to initialize OPAL_SE:', error);
  }
});
...
app.use((req: Request, res: Response, next: NextFunction) => {
  ...
  res.header('Access-Control-Allow-Headers', 'Content-Type, Accept, Mcp-Session-Id, MCP-Protocol-Version, Authorization, X-API-Token, Origin, X-Requested-With');
    ...
    ws.on('message', async (message: WebSocket.Data) => {
      ...
      try {
        ...
        if (response) {
          ...
          // Track metrics
          (metricsService as any).recordRpcRequest?.(jsonMessage.method, Date.now() - session.lastActivity, true);
        }
      } catch (error: any) {
...
function findSessionByWs(ws: WebSocket): Session | undefined {
