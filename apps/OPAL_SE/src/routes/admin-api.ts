...
router.get('/metrics', async (req: Request, res: Response) => {
  try {
    ...
    const metrics = {
      ...
      uptime: Math.floor(process.uptime()),
      version: process.env.npm_package_version || '1.0.0',
      activeConnections: (global as any).sessions ? (global as any).sessions.size : 0,
...
router.get('/resources', (req: Request, res: Response) => {
  try {
    ...
    const { resources } = resourcesService.listResources((items: any) => ({ items, nextCursor: null }), null);
...
router.get('/prompts', (req: Request, res: Response) => {
  try {
    ...
    const { prompts } = promptsService.listPrompts((items: any) => ({ items, nextCursor: null }), null);
...
router.get('/admin', async (req: Request, res: Response) => {
  try {
    ...
    const adminData = {
      ...
      connections: {
        active: (global as any).sessions ? (global as any).sessions.size : 0,
 ...
 * @access Public
 */
router.post('/test/compliance', async (req: Request, res: Response) => {
  try {
